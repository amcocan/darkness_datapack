...
This is a QoL (quality of life) datapack built for versions `1.21.3` and later of Minecraft Java Edition (MJE). Below are some of the modules that this datapack includes and some documentation on how they work.
> __Warning:__ The [Darkness ResourcePk](https://github.com/amcocan/darkness_resourcepack) is required for the best experience.


## custom recipes
The custom recipes are pretty self explanatory from just the names (_they follow the default MJE naming convention_), so I will just list them out below. Note that this will include recipes that were designed for custom modules later explained.
...
## antigrief
This module adds anti-greifing for the `minecraft:creeper`, `minecraft:enderman`, and `minecraft:ghast`.
...
> Learn about the details of this datapack on [vanillatweaks.net](https://vanillatweaks.net/picker/datapacks/) & [github](https://github.com/MukiTanuki/armor-statues#). Also, I removed the itemframe modules from this datapack since I added my own [here](#invframe).


## cleanup
This module adds the ability to bring items within 16 blocks of the player to the players location using the following `trigger` command.
...
## extrashell
This module increases the drop of shulker shells to guarantee 2 `minecraft:shulker_shell` items. This will sidestep the looting requirements from MJE.
...
## god_gear
This module removes the exclusivity of protection type enchantments and resets the repair cost of items every time you interact with an anvil. 


## invframe
This module adds the ability to _hide_, _show_, and _find_ a `minecraft:item_frame` or `minecraft:glow_item_frame`. When a `minecraft:item_frame` or `minecraft:glow_item_frame` is hidden, it is also fixed so that you don't have the item rotating or the frame breaking. When a `minecraft:item_frame` or `minecraft:glow_item_frame` is shown, it is unfixed so that you can rotate the item and break the frame. You can use this module by using the triggers below.
...
## manager
This feature allows anyone in creative mode to quickly enable/disable any module of this datapack.
...
## more_mob_heads
Originally adds a drop chance for a custom model of an entity head using the `minecraft:player_head` item. I striped out this functionality and am using part of this datapack as the back bone for the [shearheads](#shearheads) module of my datapack. Any flaws with this datapack are not the fault of [Vanilla Tweaks's Team](https://vanillatweaks.net/about/).
...
## orb_merger
This module combines experience orbs to reduce lag.


## packauthor
This module uses the `tellraw` command to provide information on the datapack every time the datapack is reloaded. It also creates the following two `trigger` commands that people can use.
- `/trigger silence_alerts` ~ _This mutes the datapack alerts for individual modules loading._
- `/trigger enable_alerts` ~ _This unmutes the datapack alerts for individual modules loading._


## playerseats
This module adds the ability to sit anywhere the player wants using the following `trigger` command.
- `/trigger sit` ~ _This makes the player sit in the location they were standing._


## shearheads
This module adds the ability to collect a `minecraft:blocks/player_head` without having to kill the player in MJE. This module also extends to other mobs/entities as well.
